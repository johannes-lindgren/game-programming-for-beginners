...
Create a function `update` that prints the current time with `Date()`.
...
At first, it might look strange that a function is being passed as an argument to another function, but this is technique that actually is very useful.

`setInterval` creates a timer that calls the function in the same argument repeatedly, with some delay. The second argument determines how long the period is in milliseconds. In the example above, `updates` gets called once per second for eternity, which means that you will see the current time getting printed out to the console every second.
...
With `setInterval` and variables (`let`) as new tools in your tool belt, print out an ever moving target:
...
=== Refactor--Units of Measurement

One obvious flaw with the code is that the game will speed up or slow down if you change the delay between each frame.

.A change like this would mean that the `2` would have to be replaced with a `4`. Imagine such a change in a larger code base, with hundreds of such values.
[source,javascript]
----
...
=== Refactor--Dynamic Frame Rate

If you've played video games, you must have noticed that the frame rate is not a constant number, but that during heavy calculations, it tends to drop. In the previous section, the delta distance time (`dt`) was calculated from a _desired_ frame rate, so that the velocity of the circle would be the same regardless of the frame rate. But there is no guarantee that the computer will actually be able to catch up with the frame rate that you set. Set the frame rate to 1000 and notice how the entire game slows down, despite the effort. This means that the delta time must be calculated dynamically--once for each frame.
